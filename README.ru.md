...
| `colorizer`  | Подсветка цветовых кодов (например, `#RRGGBB`) их цветом           |
...
1. **Установите Neovim** — требуется **Neovim ≥ 0.10** (0.11+ определяется и используется автоматически)
...
4. **Клонируйте репозиторий и удалите папку .git:**
   ...
   ```
5. **Синхронизируйте плагины** — при первом запуске Neovim `lazy.nvim` установит все плагины автоматически
...
| `[d`, `ö`                   | Предыдущая диагностика          |
| `]d`, `ä`                   | Следующая диагностика           |
| `<leader>la`                | Code Actions                    |
| `<leader>ld`, `gd`, `<C-p>` | Перейти к определению           |
| `<leader>lD`, `gD`          | Перейти к декларации            |
| `<leader>lk`, `<S-k>`       | Hover-документация              |
| `<leader>lr`, `gr`          | Показать ссылки                 |
| `<leader>lt`, `gt`, `<C-t>` | Показать определение типа       |
